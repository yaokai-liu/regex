...
enum TOKEN_TYPE_ENUM XLR_TERMINAL_TYPES[] = {Regex_TOKEN_LEFT_PARENTHESIS, Regex_TOKEN_RIGHT_PARENTHESIS,
                                             Regex_TOKEN_SPLIT, Regex_TOKEN_INVERSE, Regex_TOKEN_ASSIGNER};

inline uint32_t xlr_single_tokenize(XLRTokenizer *tokenizer, Terminal *result, const Allocator *allocator) {
  const char_t *input = tokenizer->SUPER.src + tokenizer->SUPER.offset;
  const char_t *pText = input;
  if (!*pText) {
  ...
  switch (*pText) {
    ...
    case ';': {
      if (!tokenizer->end_of_rule) {
        ...
        result->type = Regex_TOKEN_SEMICOLON;
        result->length = 1;
        result->value = nullptr;
        return REGEX_SUCCESS;
      }
...
const Terminal *xlr_tokenize(const char_t *input, uint32_t *cost, Array *ident_array, uint32_t *n_tokens,
                             ...
                             const Allocator *allocator) {  // NOLINT(*-easily-swappable-parameters)
  ...
  Array *terminals = Array_new(sizeof(Terminal), Regex_TOKEN_TOKEN, allocator);
  ...
  do {
    *cost = XLRTokenizer_next((Tokenizer *) tokenizer, &terminal, &errInfo, allocator);
    if (*cost == REGEX_SUCCESS) {
      Array_append(terminals, &terminal, 1);
    } else {
      break;
    }
  } while (terminal.type != Regex_TOKEN_TERMINATOR && tokenizer->SUPER.offset < max_cost);
